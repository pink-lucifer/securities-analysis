...
const (
	AmexCsvFile   = "../../data/listed-mkt/amex/companylist-amex.csv"
	listedMktAmex = model.AMEX
)

func ParseAmexSymbol(ch <-chan []string, eofCh <-chan int, listed chan<- *model.ListedSymbol, done chan<- int) {
...
func ParseAndWrapAmexSymbol(ch <-chan []string, eofCh <-chan int, listed chan<- *persist.TblListedSymbol, done chan<- int) {
	var total, processed int
	for {
		select {
		case line := <-ch:
			if len(line) != 0 {
				symbol, err := WrapListedSymbol(listedMktAmex, line)
				if err != nil {
					break
				}
				listed <- symbol
